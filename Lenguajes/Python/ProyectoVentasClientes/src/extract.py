...
def clean_products(df):
    ...
    df = df.groupby("Item Code", as_index=False).first()  # UNA fila por Item Code
    return df
...
def clean_wholesale(df):
    ...
    df = df.groupby(["item_code", "date"], as_index=False).agg(avg_wholesale_price=("wholesale_price", "mean"))
    return df
...
def clean_loss(df):
    ...
    df = df.groupby("item_code", as_index=False).agg(loss_rate=("loss_rate", "mean"))
    return df
...
def build_fact_table(products, sales, wholesale, loss):
    df = sales.merge(products, left_on="item_code", right_on="Item Code", how="left")
    df = df.merge(loss, on="item_code", how="left")
    df = df.merge(wholesale, on=["item_code", "date"], how="left")
...
# =========================
def main():
    print("📂 Archivos en data/raw:", os.listdir(RAW_DATA_PATH))
    ...
    output = os.path.join(PROCESSED_DATA_PATH, "fact_sales.csv")
    fact_sales.to_csv(output, index=False)

    print("✅ ETL finalizado correctamente")
    print(f"📄 Archivo generado: {output}")
    print(f"📊 Filas finales: {len(fact_sales)}")
